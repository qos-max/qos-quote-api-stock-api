...
**quote-ocean-system系统简称"QOS行情API"，包括了港股实时API,美股实时API,A股实时API，A股-京市实时API，A股-北交所实时API，加密货币实时报价API，数字币实时行情API,外汇API，贵金属API,黄金API，白银API，大宗商品API，能源API,采用REST API与websocket接口形式，非常容易接入。**</br>
**免费开源股票API、港股API、美股API、沪深股票API、A股API实时股票行情数据、A股-京市API实时股票行情数据、A股-北交所API实时股票行情数据、加密货币实时报价数据，数字币实时行情,外汇行情数据，贵金属行情数据,黄金行情报价，白银行情报价，大宗商品行情数据，能源行情数据**</br>
**QOS行情报价系统目前提供：全港，全美，沪深京市-北交所等股票和加密货币（数字币），外汇，贵金属，大宗商品，黄金，白银，能源等的实时报价，实时K线，历史K线数据等，欢迎免费试用及联系作者,现提供免费试用对接。**
...
1.4.8、[获取市场交易日历](#48获取市场交易日历)</br>
1.4.9、[获取产品列表](#49获取产品列表)</br>
...
1.5.0、[websocket协议接入说明](#50websocket协议接入说明)</br>
1.5.1、[心跳](#51心跳)</br>
1.5.2、[订阅交易品种的实时快照](#52订阅交易品种的实时快照)</br>
1.5.3、[订阅交易品种的实时逐口成交明细](#53订阅交易品种的实时逐口成交明细)</br>
1.5.4、[订阅交易品种的实时盘口](#54订阅交易品种的实时盘口)</br>
1.5.5、[订阅交易品种的K线](#55订阅交易品种的实时k线)</br>
1.5.6、[请求交易品种的实时快照](#56请求交易品种的实时快照)</br>
1.5.7、[请求交易品种的实时逐口成交明细](#57请求交易品种的实时逐口成交明细)</br>
1.5.8、[请求交易品种的实时盘口](#58请求交易品种的实时盘口)</br>
1.5.9、[请求交易品种的K线](#59请求交易品种的实时k线)</br>
1.5.10、[请求交易品种的K线历史](#510请求交易品种的k线历史)</br>
1.5.11、[请求交易品种的基础信息](#511请求交易品种的基础信息)</br>
## 2、一些枚举定义
### 2.1、K线类型
| 枚举常量 | 值   |
|----------|------|
...
- **接口地址**：`/product-list`
- **请求方式**：`POST`
- **接口描述**：获取指定一个或者多个市场的产品列表。支持全美股票，全港股票，全A（沪深）股票，外汇，贵金属，黄金，白银，指数，能源等产品。通过接口获取到的产品列表是挑选的较为热门的或者常用的，如果您需要其它的或者更多，[请联系作者](#联系作者)。
> [返回目录](#目录)

#### 4.9.1、请求参数(Body)
| 参数名     | 类型   | 必填  | 描述                        |
|------------|--------|------|-----------------------------|
| `market`  | array   |是 |市场字符串数组</br>"US": 美股</br>"HK": 港股</br>"SH": A股沪市</br>"SZ": A股深市</br>"BJ": A股京市-交交所</br>"SI": 股票指数</br>"FX": 外汇</br>"IX": 指数</br>"CM": 大宗商品（黄金，白银，能源）</br>"CF": 加密货币永续合约|
> [返回目录](#目录)
#### 4.9.2、请求示例(Body)
```json
{
...
| > `a`        | integer 复权类型 0:不复权 1:前复权        |
| > `kt`        | integer K线类型，取值参看[2.1章节](#21K线类型)K线类型定义|
| > `e`        | integer 请求K线的结束时间戳，单位秒,忽略此字段，需要更多历史K线数据请联系客服另外提供。                 |
| `reqid`    | integer类型。可选。请求id，由客户端传入，服务器会原值返回，用于客户端自己索引一条唯一的请求和返回。客户端结合自身需要考虑是否传入合适的值。 |
> [返回目录](#目录)
#### 5.10.2、请求命令示例
```json
