...
#### 1.2.1、[K线类型](#21k线类型)
#### 1.2.2、[交易方向](#22交易方向)
### 1.3、[查看各类交易品种的编码和交易时间](#3各类交易品种的编码和交易时间)
### 1.4、请求Http接口
把key加入到**URL参数中**或者**请求头中**使用key字段填充，然后根据http的各个接口定义的参数就可以获取数据了。
#### 1.4.1、[http协议接入说明](#40http协议接入说明)
#### 1.4.2、[获取交易品种的基础信息](#42获取交易品种的基础信息)
#### 1.4.3、[获取交易品种的实时行情快照](#43获取交易品种的实时行情快照)
#### 1.4.4、[获取交易品种的实时最新盘口深度](#44获取交易品种的实时最新盘口深度)
#### 1.4.5、[获取交易品种的实时最新逐笔成交明细](#45获取交易品种的实时最新逐笔成交明细)
#### 1.4.6、[获取交易品种的K线](#46获取交易品种的K线)
#### 1.4.7、[获取交易品种的K线-历史](#47获取交易品种的K线历史)
### 1.5、订阅webscoket实时数据接口
把key加入到**URL参数中**或者**请求头中**使用key字段填充，然后建立websocket连接，发送相应的订阅命令，并保持心跳就可以得到实时报价了
#### 1.5.1、[websocket协议接入说明](#50websocket协议接入说明)
#### 1.5.2、[心跳](#51心跳)
#### 1.5.3、[订阅交易品种的实时快照](#52订阅交易品种的实时快照)
#### 1.5.4、[订阅交易品种的实时逐口成交明细](#53订阅交易品种的实时逐口成交明细)
#### 1.5.5、[订阅交易品种的实时盘口](#54订阅交易品种的实时盘口)
#### 1.5.6、[订阅交易品种的K线](#55订阅交易品种的实时k线)
#### 1.5.7、[请求交易品种的实时快照](#56请求交易品种的实时快照)
#### 1.5.8、[请求交易品种的实时逐口成交明细](#57请求交易品种的实时逐口成交明细)
#### 1.5.9、[请求交易品种的实时盘口](#58请求交易品种的实时盘口)
#### 1.5.10、[请求交易品种的K线](#59请求交易品种的实时k线)
#### 1.5.11、[请求交易品种的K线历史](#510请求交易品种的k线历史)
## 2、一些枚举定义
### 2.1、K线类型
| 枚举常量 | 值   |
|----------|------|
